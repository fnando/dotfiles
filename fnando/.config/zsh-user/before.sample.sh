# Set your Git commit info.
# export GIT_AUTHOR_NAME="Your name"
# export GIT_AUTHOR_EMAIL="Your e-mail"
# export GIT_AUTHOR_SIGNING_KEY="pbcopy < ~/.ssh/id_ed25519.pub"
# export GIT_COMMITTER_NAME=$GIT_AUTHOR_NAME
# export GIT_COMMITTER_EMAIL=$GIT_AUTHOR_EMAIL

# x86_64 (Intel)
# export GIT_DIFF_HIGHLIGHT="/usr/local/opt/git/share/git-core/contrib/diff-highlight/diff-highlight"

# arm64 (Apple Silicon)
# export GIT_DIFF_HIGHLIGHT="/opt/homebrew/opt/git/share/git-core/contrib/diff-highlight/diff-highlight"

# If you use Sublime Text
# source ~/.config/zsh/sublime-text.sh

# If you use VSCode
# source ~/.config/zsh/vscode.sh

# Load 1password-cli plugin.
# Must be loaded after all environment variables are set.
# Must be loaded before git config.
# source ~/.config/zsh/1password.sh

# Load Git config.
# source ~/.config/zsh/git.sh
